...
// CheckFeasible: Check whether the current solution is feasible
// @param prob  Pointer to the problem
void __stdcall CheckFeasible( UFFProblem* prob )
{
   double x_i_j, y_j;
   std::stringstream s, message;
   std::string xName, yName, cutName;
...
int main( int argc, char* argv[] )
{
   ...
   UFFLP_WriteLP( prob, "FacLoc_incomplete.lp" );

   // Configure the log file and the log level = 2
   UFFLP_SetLogInfo( prob, "FacLoc_incomplete.log", 2 );

   // Set the cut generation callback
   UFFLP_SetCutCallBack( prob, CutGenerator );

   // Set the ineteger solution check callback
   UFFLP_SetIntCheckCallBack( prob, CheckFeasible );
